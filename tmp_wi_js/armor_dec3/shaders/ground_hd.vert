...
void main()
{
    ...
    vec4 worldPos = u_invViewProjectionMatrix * gl_Position;    // near clip plane in space
    v_toAtmosphere = normalize(worldPos.xyz / worldPos.w - u_cameraPosition);      // ray to atmosphere
    v_toAtmosphere.y = -v_toAtmosphere.y;
    ...
    float k = v_toAtmosphere.y * fInnerRadius;
    ...
    float fDepth = 1.0;
    float fStartAngle = v_toAtmosphere.y;
    float fStartOffset = fDepth*scale(fStartAngle);

    // Initialize the scattering loop variables
    float fSampleLength = fFar / fSamples;
    float fScaledLength = fSampleLength * fScale;
    vec3 v3SampleRay = v_toAtmosphere * fSampleLength;
    ...
    for(int i=0; i<nSamples; i++)
    {
        ...
        float fCameraAngle = dot(v_toAtmosphere, v3SamplePoint) / fHeight;
